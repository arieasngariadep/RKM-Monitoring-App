...
<ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active">List BND Payment</li>
</ol>
@endsection('breadcrumbs')

@section('titleTab', 'List BND Payment')
@section('title', 'List BND Payment')
...
<div class="row">
    <div class="col-lg-12 col-sm-12">
        <div class="card">
            <div class="card-body">
                <p class="text-muted mb-3">List BND Payment</p> 
                <form action="<?= route('getListKartuKredit') ?>" method="GET">
                    <div class="form-material">
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    <input autocomplete="off" type="text" id="txn_dte" class="form-control" placeholder="Txn Date" name="txn_date" value="<?= $txn_date ?>" />
                                </div>
                            </div><!--end col-->
                            <div class="col-md-1">
                                <div class="form-group">
            ...
            <div class="row mb-4">
                <div class="col-md-5 text-right">
                    <a type="button" class="btn btn-success btn-square btn-outline-dashed waves-effect waves-light"  href="../Excel/format_searchbulk_KK.xlsx"><i class="dripicons-cloud-download mr-2"></i>&nbsp;Download Format</a>
                </div>
                <div class="col-md-2 text-center">
                    <button class="btn btn-dark" style="width: 200px; height: 38px;" href="" data-toggle="modal" data-animation="bounce" data-target=".search-bulk-kk"><i class="dripicons-search"></i>&nbsp;&nbsp;Search Bulk</button>
                </div>
                <div class="col-md-5 text-left">
                    <form action="<?= route('prosesExportReportSearchBulkKK') ?>" method="POST">
                        @csrf
            ...
            <div class="card-body table-responsive">
                <table class="table table-bordered mb-0 table-centered">
                    ...
                    @if (is_array($listKK) || is_object($listKK))
                    <tbody>
                        @if(count($listKK) > 0)
                        @foreach($listKK as $val)
                        <tr>
                            <td>{{ $val->mid }}</td>
                            <td>{{ $val->mname }}</td>
                            <td>{{ $val->bankName }}</td>
                ...
                <div class="row">
                    <div class="col-md-5">
                        @if(count($listKK) > 0)
                        <div class="pull-left">
                            Showing {{ $listKK->firstItem() }} to {{ $listKK->lastItem() }} of {{ number_format($listKK->total()) }} entries
                        </div>
                        @else
                        <div class="pull-left">
                            Showing 0 to 0 of {{ $listKK->total() }} entries
                        </div>
                        @endif
                    </div>
                    <div class="col-md-7">
                        <div class="pull-right">
                            {{ $listKK->links('layout.pagination') }}
                        </div>
                    </div>
                </div>
                @endif
